...
public class StartupWeatherDAO extends AbstractMybatis {
	...
	/* 시군구별 평균 운영연수 */
	/*
	 * public List<OperateAvgDTO> getOperateAvg(String guCode) { SqlSession
	 * sqlSession = getSqlSessionFactory().openSession(); List<OperateAvgDTO>
	 * result; try { String statement = namespace + ".getOperateAvg";
	 * map.put("guCode", guCode); result = sqlSession.selectList(statement, map);
	 * sqlSession.commit(); } finally { sqlSession.close(); } return result; }
	 */
