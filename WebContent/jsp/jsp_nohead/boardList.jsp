...
<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core"%>
   ...
   <div class="list-box">
      <c:if test="${count == 0 }">
         <h1 class="empty-txt">첫 게시물을 등록해보세요</h1>
      </c:if>
      <c:if test="${count != 0 }">
   ...
   <div class="reply-reg-box">
      <form method="post" id="commentform${article.boardid}">
         <input type="hidden" name="userid" id="userid" value="${userid}" />
         <input type="hidden" name="name" id="name" value="${name }" />
         <input type="hidden" name="boardid" id="board_id" value="${article.boardid}" />
         <input type="text" id="content${article.boardid}" name="content" placeholder="댓글을 입력하세요" />
         <div class="reg-btn"
            onclick="javascript:writeComment(${article.boardid});">
            <svg viewBox="0 0 40 40" class="reg-icon">
               <path
